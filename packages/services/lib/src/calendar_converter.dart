...
class CalendarConverter {
  //intVal(x) để lấy số nguyên lớn nhất không vượt quá x
  static int intVal(double value) {
    return value.floor();
  }
  ...
  static int jdFromDate(int dd, int mm, int yy) {
    ...
    jd = dd +
        ...
        32045;
    if (jd < 2299161) {
      jd = dd + intVal((153 * m + 2) / 5) + 365 * y + intVal(y / 4) - 32083;
    }
    return jd;
  }
  ...
  static int getSunLongitude(jdn, timeZone) {
    ...
    L = L * dr;
    L = L - pi * 2 * (intVal(L / (pi * 2))); // Normalize to (0, 2*PI)
    return intVal(L / pi * 6);
  }
  ...
  static int getLeapMonthOffset(int a11, int timeZone) {
    var k, last, arc, i;
    k = intVal((a11 - 2415021.076998695) / 29.530588853 + 0.5);
    last = 0;
  ...
  static List<int> solarToLunar(int solarYear, int solarMonth, int solarDay, Timezone timezone) {
    ...
    var diff = intVal((monthStart - a11) / 29);
    lunarLeap = 0;
    lunarMonth = diff + 11;
    if (b11 - a11 > 365) {
  ...
  List<int> lunarToSolar(
      int lunarYear, int lunarMonth, int lunarDay, int lunarLeap, Timezone timezone) {
    List<int> result = new List.filled(3, 0);
    ...
    k = intVal(0.5 + (a11 - 2415021.076998695) / 29.530588853);
