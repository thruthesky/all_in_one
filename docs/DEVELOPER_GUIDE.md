...
- 루트(최상위) 프로젝트인 all_in_one 을 VSCode 에서 열어 개별 프로젝트 작업을 할 때, 동일한 이름의 소스 파일과 코드가 많아 작업하기가 불편하다.
  - 이 때, 개별 프로젝트만 VSCode 에서 열어 작업을 해도 된다. 물론, 이 때, 최상위 코드를 수정하면, 앱을 hard reload 해야 한다.
